...
Pengujian fungsional bertujuan untuk memastikan setiap fitur baru pada SharIF Judge dapat berfungsi dengan baik.
	...
	Hasil yang diharapkan dari pengujian fitur ini adalah pengguna dapat \textit{login} ke \textit{SharIF Judge} menggunakan \textit{username} yang terdaftar pada \textit{server RADIUS}. \textit{Username} yang digunakan dalam pengujian fitur ini adalah xxxxx. Pengujian dimulai dengan \textit{login} menggunakan \textit{username} xxxxxx ke perangkat lunak \textit{SharIF Judge} yang telah diinstal pada \textit{server} lab FTIS UNPAR (selanjutnya digunakan untuk pengujian eksperimental). \textit{SharIF Judge} yang telah diinstal, dapat diakses pada \textit{URL} \path{http://asd.labftis.net/}. \hyperref[fig:unduh]{Gambar 5.28} menunjukan bahwa \textit{username} xxxxx berhasil \textit{login} ke \textit{SharIF Judge}.
	\begin{figure}[H]
		\centering  
